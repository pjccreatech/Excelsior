...
namespace CTechCore
{
    public class MyApp
    {
        ...
        public static Con Common = Cons["CmmnTest"];
#else
        
        public static Con Evo = Cons["EvoLive"];
        public static Con CTech = Cons["RollsLive"];
        public static Con Common = Cons["CmmnLive"];
#endif
        public static Pastel.Evolution.ComHelper PastelHlpr = new Pastel.Evolution.ComHelper();

        public static Settings Settings = new Settings();

        public static CTechCore.Tools.frmEvents EventsScreen = new CTechCore.Tools.frmEvents();
        ...
        public static CTechCore.Tools.cLog Log
        {
            get { return new CTechCore.Tools.cLog(); }
        }
    ...
    public class Con : ConfigurationElement
    {
        ...
        public bool ExecSQL(string sSQL, ref DataSet ds, List<Params> parms)
        {
            try
            {
                if (this.SQLCon != null)
        ...
        {
            get { return $"Persist Security Info=True;Password={this.Password};User ID={this.Username};Data Source={this.Server};Initial Catalog={this.Database};MultipleActiveResultSets=true; Connection Timeout={this.ConnectionTimeOut}"; }
    ...
    public class Setting
    {
        ...
        public Setting(Int64 id)
        {
            try
            {
                MyApp.Settings.Refresh();
                CTechCore.Setting sets = MyApp.Settings[(int)id];
                if (sets != null)
                {
                    this.ID = sets.ID;
                    this.App = sets.App;
                    this.HeaderSection = sets.HeaderSection;
                    this.HeaderKey = sets.HeaderKey;
        ...
        public override string ToString() => $"{this.HeaderSection} - {this.HeaderKey}";
        public string DisplayText { get{ return $"{this.HeaderSection} - {this.HeaderKey}"; } }
    ...
    public class Settings : System.ComponentModel.BindingList<Setting>, System.Collections.ICollection, System.Collections.IEnumerable
    {
        ...
        public void Refresh()
        {
            try
            {
                this.Clear();
                DataTable dt = new DataTable();
                MyApp.CTech.ExecSQL($"SELECT * from XR_Config WHERE App = '{MyApp.Name}' ", ref dt);
                foreach (DataRow dr in dt.Rows)
                {
        ...
        new public Setting this[int id]
        {
            get
            {
                this.Refresh();
